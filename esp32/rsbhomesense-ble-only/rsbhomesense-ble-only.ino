...
void loop() {
  ...
  // if NOT connected and we time out, go to sleep OR we are connected AND delayTime marked to execute once
  if((!deviceConnected && bleRetries > 30) || (deviceConnected && delayTime == -1 && bleRetries > 3)) {
    // if no bluetooth connection, shutdown
    Serial.println("Shutting down...");
    if(!deviceConnected && bleRetries > 60) {
      Serial.println("For long sleep...");
      default_sleep_in_minutes = 60 * 15;   // 15 minutes
    }

    doShutdown();
  }

  // code won't get here unless deviceConnected == true
